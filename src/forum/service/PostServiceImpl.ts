...
export default class PostServiceImpl implements PostService {
    async createPost(author: string, title: string, content: string, tags: Set<string>): Promise<PostDto> {
        ...
        return new PostDto(res.id, res.title, res.content,
                        res.author, res.dateCreated,
                        Array.from(res.tags), res.likes, res.comments.map(c => c as unknown as CommentDto));
    }
    ...
    async findPostById(id: string): Promise<PostDto> {
        ...
        return new PostDto(post.id, post.title, post.content,
            post.author, post.dateCreated,
            Array.from(post.tags), post.likes, post.comments.map(c => c as unknown as CommentDto));
    }

    async addComment(id: string, user: string, message: string): Promise<PostDto> {
        const post = await P.findByIdAndUpdate(id,
            {
                $push: {
                    comments: {
                        user,
